use std::{io::{Read, Cursor}, path::{Path}, collections::VecDeque, cell::RefCell, ffi::OsStr};
...
pub fn get_contents_from_zip_file(paths: Vec<String>, zip_archive: RefCell<ZipArchive<Cursor<Vec<u8>>>>, mt_d_file_contents: &MtDeque<Option<String>>) {
    for path in paths {
        let mut zip_archive_borrow = zip_archive.borrow_mut();
        let mut zip_file = match zip_archive_borrow.by_name(&path) {
            Ok(v) => v,
            Err(_) => continue,
        };
...
pub fn get_file_names_from_zip_archive(zip_archive: RefCell<ZipArchive<Cursor<Vec<u8>>>>, mt_d_filenames: &MtDeque<Option<FileForIndex>>) {
    ...
    loop {
        ...
        let regular_file_names = popped_zip_archive.borrow_mut()
                                                        .file_names()
                                                        .filter(|s| {
                                                            let p = Path::new(s);
        ...
        let zip_archive_file_names = popped_zip_archive.borrow_mut()
                                                        .file_names()
                                                        .filter(|s| {
                                                            let p = Path::new(s);
