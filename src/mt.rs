...
impl<T: RealSize> MtDeque<T> {
    ...
    pub fn push_back(&self, elem: T) {
        ...
        let mut guard = self
            ...
            .wait_while(self.deque.lock().unwrap(), |_| {
                self.deque_size_bytes.load(std::sync::atomic::Ordering::Relaxed) + elem_real_size >= DEQUE_SIZE_LIMIT_BYTES
            })
            .unwrap();

        guard.push_back(elem);
        self.deque_size_bytes.fetch_add(elem_real_size, std::sync::atomic::Ordering::Relaxed);
    ...
    pub fn pop_back(&self) -> T {
        ...
        self.deque_size_bytes.fetch_sub(popped_elem.get_real_size(), std::sync::atomic::Ordering::Relaxed);
        ...
        match self {
            Some(file_for_index) => {
                match file_for_index {
                    ...
                    FileForIndex::Zip(paths, _) => {
                        paths.len() * std::mem::size_of::<String>()
                    }
                }
            },
